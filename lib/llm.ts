...
export async function generateTreasure(
  topic: string,
  hops: { title: string; highlight: string }[],
  bridges: string[]
): Promise<{ summary: string; hiddenPattern: string; whyOpen: string }> {
  const pathDescription = hops
    .map(
      (h, i) =>
        `${i === 0 ? "Origin" : `Hop ${i}`}: "${h.title}" — ${h.highlight}`
    )
    .join("\n");
  const bridgeDescription = bridges
    .map((b, i) => `Bridge ${i + 1}: ${b}`)
    .join("\n");
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You analyze a conceptual drift path — a chain of surprising connections from one domain to another — and reveal the hidden pattern.
Return ONLY a JSON object with:
- "summary": one sentence summarizing the entire journey
- "hiddenPattern": the deep, non-obvious pattern that connects all the hops (1-2 sentences)
- "whyOpen": why someone should explore the final destination further (1 sentence)`,
      },
      {
        role: "user",
        content: `Starting topic: "${topic}"\n\nPath:\n${pathDescription}\n\nBridges:\n${bridgeDescription}`,
      },
    ],
  });
  ...
  try {
    ...
    return {
      summary: `A journey from ${topic} to unexpected territories`,
      hiddenPattern:
        "The same fundamental patterns appear across seemingly unrelated domains",
      whyOpen: "Every field has something to teach the others",
    };
  }
}
